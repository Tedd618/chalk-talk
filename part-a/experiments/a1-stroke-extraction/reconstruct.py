...
def reconstruct(tag: str, fps: int = 30, color_mode: str = "single",
    ...
    # Use the page number embedded in each stroke (v4 extraction).
    # Fall back to pages.jsonl timestamps if page field is absent (older data).
    has_page_field = all("page" in s for s in strokes)
    ...
    if not has_page_field:
        ...
        if page_break_times:
            print(f"[{tag}] honoring {len(page_break_times)} page break(s) from pages.jsonl")
    else:
        n_pages = max(s["page"] for s in strokes) + 1
        print(f"[{tag}] honoring {n_pages} page(s) from stroke page field")
    ...
    def current_page_number(t: float) -> int:
        """Which page is active at time t, based on stroke timestamps."""
        # Page number = page of the most recent stroke that started before t
        page = 0
        for s in strokes:
            if s["t_start"] <= t:
                page = s.get("page", 0)
    ...
    # Pre-build per-stroke arrays for speed
    stroke_arrays: list[tuple[int, int, list[tuple[float, float, float]]]] = [
        (s["stroke_id"],
         s.get("page", 0),
    ...
    for i in range(n_frames):
        ...
        if has_page_field:
            active_page = current_page_number(t)
        else:
            page_start = current_page_start(t)

        for sid, page_num, pts in stroke_arrays:
            if not pts or pts[0][0] > t:
                continue
            # Filter by page: only show strokes on the current page
            if has_page_field:
                ...
                if pts[0][0] < page_start:
                    continue
            shown_xy = [(x, y) for tt, x, y in pts]  # show full stroke once started
            if len(shown_xy) < 2:
                if len(shown_xy) == 1:
